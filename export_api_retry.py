...
# Step 1: Fetch all reports and filter (same as v3)
print("[1/4] Fetching reports...", flush=True)
t0 = time.time()
...
# Step 2: Load existing Excel to find which reports we already have
print("[2/4] Loading existing Excel to find missing reports...", flush=True)
existing_rids = set()
try:
    wb_old = openpyxl.load_workbook(OUTPUT, read_only=True)
    ws_old = wb_old["BASE PREST (API)"]
    for row in ws_old.iter_rows(min_row=2, max_col=1, values_only=True):
        if row[0]:
            existing_rids.add(row[0])
    wb_old.close()
    print(f"  Already have {len(existing_rids)} reports in Excel", flush=True)
except Exception as e:
    print(f"  Could not load Excel: {e}", flush=True)

missing = [r for r in filtered if r["id"] not in existing_rids]
print(f"  Missing: {len(missing)} reports to fetch", flush=True)

if not missing:
    print("  Nothing to do! Excel is complete.", flush=True)
    exit(0)

# Step 3: Fetch missing expenses with 2 workers (gentle to avoid 429)
print(f"[3/4] Fetching {len(missing)} missing reports ({MAX_WORKERS} workers)...", flush=True)
expenses_all = []
errors = []
lock = threading.Lock()
done_count = 0
    ...
    for attempt in range(5):
        try:
            ...
            if r.status_code == 429:
                with lock:
                    errors.append(f"Report {rid}: 429")
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    futures = {executor.submit(fetch_expenses, r): r for r in missing}
    for future in as_completed(futures):
        result = future.result()
        with lock:
            expenses_all.extend(result)
            done_count += 1
            if done_count % 25 == 0 or done_count == len(missing) or (time.time() - last_log) > 15:
                elapsed = time.time() - t1
                pct = done_count / len(missing) * 100
                rate = done_count / elapsed * 60 if elapsed > 0 else 0
                eta = elapsed / done_count * (len(missing) - done_count) if done_count > 0 else 0
                print(f"  [{done_count}/{len(missing)}] {pct:.0f}% | {len(expenses_all)} expenses | {rate:.0f} rep/min | {elapsed:.0f}s, ETA {eta:.0f}s | err={len(errors)}", flush=True)
                last_log = time.time()

print(f"  Fetched {len(expenses_all)} expenses from {len(missing)} reports | errors={len(errors)}", flush=True)

# Step 4: Merge with existing Excel and rebuild
print("[4/4] Merging with existing Excel...", flush=True)

# Load all existing expenses
wb_old = openpyxl.load_workbook(OUTPUT, read_only=True)
ws_old = wb_old["BASE PREST (API)"]
h3 = [c.value for c in ws_old[1]]
existing_expenses = []
for row in ws_old.iter_rows(min_row=2, values_only=True):
    if row[0]:
        existing_expenses.append({
            "report_id": row[0], "report_name": row[1], "status": row[2],
            "cpf": row[3], "nome": row[4], "approval_date": row[5],
            "report_total": row[6], "expense_id": row[7], "description": row[8],
            "value": float(row[9] or 0), "date": row[10], "expense_status": row[11],
            "payment_method_id": str(row[12] or ""), "payment_method_name": row[13],
        })
wb_old.close()

all_expenses = existing_expenses + expenses_all
all_expenses.sort(key=lambda e: (e["cpf"], e["report_id"], e["expense_id"] or 0))
print(f"  Total: {len(all_expenses)} expenses ({len(existing_expenses)} old + {len(expenses_all)} new)", flush=True)
...
for col, h in enumerate(h3, 1):
    ...
    c.font = header_font; c.fill = header_fill; c.alignment = Alignment(horizontal='center'); c.border = thin_border
for i, e in enumerate(all_expenses, 2):
...
ws3.auto_filter.ref = f"A1:N{len(all_expenses)+1}"; ws3.freeze_panes = "A2"

wb.save(OUTPUT)
total_val = sum(e["value"] for e in all_expenses)
print(f"DONE! {len(all_expenses)} expenses, {len(report_summary)} reports, {len(cpf_summary)} CPFs, Total: R$ {total_val:,.2f}", flush=True)
