...
# Split into same_report vs different_report
...
def write_analysis(ws, stats, label, start_row=1):
    ...
    # By user (top 20)
    ws.cell(row=row, column=1, value='Top Usuários por Valor').font = subtitle_font
    row += 1
    for col, h in enumerate(['Usuário', 'Qtd Pares', 'Valor Total'], 1):
        c = ws.cell(row=row, column=col, value=h)
        c.font = header_font_white
        c.fill = header_fill
    row += 1
    for user, d in list(stats['by_user'].items())[:20]:
...
ws1 = wb.active
ws1.title = 'Todas'
next_row = write_analysis(ws1, stats_all, 'Todas as Duplicatas 2026+')
ws1.cell(row=next_row, column=1, value='Detalhamento').font = subtitle_font
next_row += 1
next_row = write_detail_header(ws1, next_row)
write_detail_rows(ws1, pairs, next_row)

# Tab 2: Different reports
ws2 = wb.create_sheet('Relatórios Diferentes')
next_row = write_analysis(ws2, stats_diff, 'Duplicatas em Relatórios Diferentes 2026+')
ws2.cell(row=next_row, column=1, value='Detalhamento').font = subtitle_font
next_row += 1
next_row = write_detail_header(ws2, next_row)
write_detail_rows(ws2, diff_report, next_row)

# Tab 3: Same report
ws3 = wb.create_sheet('Mesmo Relatório')
next_row = write_analysis(ws3, stats_same, 'Duplicatas no Mesmo Relatório 2026+')
ws3.cell(row=next_row, column=1, value='Detalhamento').font = subtitle_font
next_row += 1
next_row = write_detail_header(ws3, next_row)
write_detail_rows(ws3, same_report, next_row)
...
def add_analysis_to_doc(doc, stats, label):
    ...
    doc.add_heading('Top Usuários por Valor', level=2)
    users = list(stats['by_user'].items())[:15]
    table = doc.add_table(rows=len(users) + 1, cols=3, style='Table Grid')
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    headers = ['Usuário', 'Qtd Pares', 'Valor Total']
    for j, h in enumerate(headers):
        cell = table.rows[0].cells[j]
        cell.text = h
        for p in cell.paragraphs:
...
doc.add_paragraph(f'Os {total} pares de duplicatas estão listados no arquivo Excel anexo com todos os campos detalhados.')
doc.add_paragraph(f'Arquivo: duplicatas_2026_report.xlsx')
doc.add_paragraph('')
