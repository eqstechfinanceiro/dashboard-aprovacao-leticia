...
const MIN_INTERVAL_BETWEEN_CALLS = 2000;
...
export async function processReceiptImageGroq(
  imageUrl: string,
  apiKey: string,
  maxRetries = 1
  ...
  try {
    ...
    return processReceiptImageGroqBase64(imageBase64, mimeType, apiKey, maxRetries);
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    if (msg.includes('timeout') || msg.includes('aborted')) {
      return { success: false, error: 'Timeout requesting Groq API' };
    }
    return { success: false, error: msg };
  }
}

export async function processReceiptImageGroqBase64(
  imageBase64: string,
  mimeType: string,
  apiKey: string,
  maxRetries = 1
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      const elapsed = Date.now() - lastGroqCallTime;
      if (elapsed < MIN_INTERVAL_BETWEEN_CALLS) {
        const wait = MIN_INTERVAL_BETWEEN_CALLS - elapsed;
        console.log(`[Groq] Rate limit protection, waiting ${wait}ms before call...`);
        await sleep(wait);
      }
      ...
      if (response.status === 429 || response.status === 503) {
        ...
        console.log(`[Groq] API ${response.status}, attempt ${attempt + 1}/${maxRetries}, waiting ${waitTime}ms...`);
        await sleep(waitTime);
        if (attempt < maxRetries - 1) continue;
      } else {
        const errorText = await response.text();
        return { success: false, error: `Groq API error ${response.status}: ${errorText.slice(0, 200)}` };
      }
    }

    if (!response || response.status !== 200) {
      return { success: false, error: `Groq API error after ${maxRetries} attempts` };
    }
    ...
    try {
      ...
      if (start >= 0 && end > start) {
        ...
        return { success: false, error: `Could not parse JSON from Groq: ${text.slice(0, 200)}` };
      }
    }
