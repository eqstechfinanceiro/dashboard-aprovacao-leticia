...
let cachedToken: { token: string; session: string | null; expiresAt: number } | null = null;

export async function getLaravelToken(): Promise<{ token: string; session: string | null } | null> {
  if (!sql) return null;

  if (cachedToken && cachedToken.expiresAt > Date.now()) {
    return { token: cachedToken.token, session: cachedToken.session };
  }

  try {
    const rows = await sql`
      SELECT laravel_token, laravel_session, expires_at
    ...
    return { token: row.laravel_token, session: row.laravel_session };
  } catch {
    return null;
  }
}
