...
function analyzeSheetDetailed(filePath, sheetName) {
  ...
  // Contar valores únicos na coluna de quinzena
  const quinzenaCol = 4; // Coluna QUINZENA
  const quinzenas = {};
  for (let i = 2; i < jsonData.length; i++) {
    const row = jsonData[i];
    if (row && row[quinzenaCol]) {
      const qz = row[quinzenaCol];
      quinzenas[qz] = (quinzenas[qz] || 0) + 1;
    }
  }

  console.log(`\nDISTRIBUIÇÃO POR QUINZENA:`);
  Object.entries(quinzenas).forEach(([qz, count]) => {
    console.log(`  ${qz}: ${count} registros`);
  });
}
...
function main() {
  ...
  // Analisar sheet QUINZENAS
  analyzeSheetDetailed(planilha2Path, 'QUINZENAS');

  // Analisar sheet ADICIONAIS
  analyzeSheetDetailed(planilha2Path, 'ADICIONAIS');
