...
export default function FechamentoPage() {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      setSyncMessage(`Sincronizado: ${json.reportsSynced} relatórios, ${json.expensesSynced} despesas`);
        ...
        <CardContent className="pt-6">
          ...
          {selectedUserId && (
            <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
              ...
              {syncMessage && (
                <span className={syncMessage.startsWith('Erro') ? 'text-red-600' : 'text-green-600'}>
                  {syncMessage}
                </span>
              )}
            </div>
