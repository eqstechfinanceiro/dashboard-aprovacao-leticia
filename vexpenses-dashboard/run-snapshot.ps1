$token = "eyJhbGciOiJIUzI1NiJ9..qlE_2tIdygTy22zSimERZboPUrt0ohv17HRAUac4e8s"
$session = New-Object Microsoft.PowerShell.Commands.WebRequestSession
$cookie = New-Object System.Net.Cookie("vexp_auth_token", $token, "/", "localhost")
$session.Cookies.Add($cookie)

$quinzena = $args[0]
if (-not $quinzena) { $quinzena = "2026-06-2" }

$body = @{ quinzena = $quinzena; step = "snapshot_somase" } | ConvertTo-Json
Write-Host "Triggering snapshot_somase for $quinzena..."
try {
    $r = Invoke-RestMethod -Uri "http://localhost:3000/api/pipeline/step" -WebSession $session -Method POST -Body $body -ContentType "application/json" -TimeoutSec 300
    Write-Host "Result:" -ForegroundColor Green
    $r | ConvertTo-Json -Depth 5
} catch {
    Write-Host "ERROR: $($_.Exception.Message)" -ForegroundColor Red
}
