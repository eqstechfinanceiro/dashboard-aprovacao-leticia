...
def analyze_saldo_patterns(all_data):
    ...
    if saldo_final_ratios:
        ...
        print(f"  Exemplos:")
        for i, data in enumerate(relevant_data[:5]):
            if data['quinzena_qz'] > 0:
    ...
    # Análise 4: Buscar por valores fixos
    print(f"\nANÁLISE 4: BUSCAR POR VALORES FIXOS")
    
    saldo_final_values = [d['saldo_final'] for d in relevant_data if d['saldo_final'] > 0]
    saldo_cartao_values = [d['saldo_cartao'] for d in relevant_data if d['saldo_cartao'] > 0]
    
    print(f"  Valores únicos SALDO FINAL: {len(set(saldo_final_values))}")
    print(f"  Valores únicos SALDO CARTAO: {len(set(saldo_cartao_values))}")
    
    # Mostrar valores mais comuns
    from collections import Counter
    
    if saldo_final_values:
        common_final = Counter(saldo_final_values).most_common(5)
        print(f"  SALDO FINAL mais comum: {common_final}")
    
    if saldo_cartao_values:
        common_cartao = Counter(saldo_cartao_values).most_common(5)
        print(f"  SALDO CARTAO mais comum: {common_cartao}")
    ...
    return {
        ...
        'saldo_final_common_values': Counter(saldo_final_values).most_common(10) if saldo_final_values else [],
        'saldo_cartao_common_values': Counter(saldo_cartao_values).most_common(10) if saldo_cartao_values else []
    }

def main():
    """Função principal"""
    print("DESCOBRINDO PADRÕES DE SALDO")
    print("="*80)
    ...
    if patterns['saldo_final_avg_ratio'] > 0:
        results['conclusion'] = 'PADRÕES MATEMÁTICOS ENCONTRADOS'
        print(f"\n✅ PADRÕES ENCONTRADOS:")
        ...
        results['conclusion'] = 'NENHUM PADRÃO CLARO ENCONTRADO'
    
    output_file = '/home/haumea/Projects/dashboard-aprovacao-leticia/investigation-docs/saldo_patterns.json'
